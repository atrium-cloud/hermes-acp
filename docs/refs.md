...
- Pinned reference: Hermes 0.21.3 (tag `v2026.9.14`), the version `src/gateway/types.ts` was hand-verified against (payload shapes, method params/results, turn lifecycle, and the server→client request contracts, 2026-09-21). Every bump re-verifies the typed subset, then moves this reference.
- Supported: 0.21.3 and above. 0.21.3 replaced the `approval.request`/`clarify.request` events and the `*.respond` methods with JSON-RPC server→client requests (commit ebe8cda8), so approvals and clarify questions do not reach this adapter on older builds. Hermes is an external runtime dependency the way codex-acp's `@openai/codex` and claude-agent-acp's `@anthropic-ai/claude-agent-sdk` are, except it is a Python install (`install.sh`/uv) that cannot ride in the package or the release bundle. Nothing is checked at runtime — no version floor, no `desktop_contract` gate; `_session_info.version`/`desktop_contract` stay typed because they are on the wire, unread.
- Verified against live Hermes 0.21.3 in a Fly.io sprite E2E (2026-09-21, turn lifecycle tier including the terminal-entry case); 0.20.5 in a cloud Linux VM E2E (2026-08-24).
...
- `tui_gateway/server_requests.py` — server→client requests: `{id: "srq-…", method, params: {session_id, …}}` frames answered by a response frame on the same id; `request.cancel {id, method, reason}` withdraws one; `open_requests` on `session.resume`/`session.activate`. Contracts (params/result shapes per method) in `tui_gateway/contracts/server_requests.py`; the whole wire registry in `tui_gateway/contracts/`.
